...
def make_safe_table_name(raw: str) -> str:
    ...
    prefix = generate(alphabet="_abcdefghijklmnopqrst", size=4).lower()
...
async def drop_table(
    ...
    try:
        duck_db_conn.execute(f'DROP TABLE "{safe_name}"')
        return {
            "status": "success",
            "message": (f"Table '{safe_name}' dropped successfully"),
        }
...
async def clear_all_tables(
    ...
    for (tbl_name,) in tables:
        try:
            duck_db_conn.execute(f'DROP TABLE "{tbl_name}"')
            dropped.append(tbl_name)
        except Exception as e:
            logger.error(
                "Error dropping table {table_name}: {e}",
                table_name=tbl_name,
                e=e,
            )
